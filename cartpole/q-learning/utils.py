...
def discretize_state(state, limits, no_bins):
	...
	for i, coord in enumerate(state):
		...
		if coord < limits[i][0]:
			...
			bin_index = np.floor(no_bins*scale_portion)

		# disc_state[i] = bin_index*(total_range/no_bins[i]) + limits[i][0]
		disc_state[i] = int(bin_index)

	return tuple(disc_state)


def update_q_table(Q_table, new_state, current_state, action, reward, lr, disc_factor=0.95):
	"""
	Updates Q table based on rewards using bellman's equation.
	"""
	learned_Q_val = reward + disc_factor*np.max(Q_table[new_state[0], new_state[1]])
	old_Q_val = Q_table[current_state[0], current_state[1], action]

	Q_table[current_state[0], current_state[1], action] = (1 - lr)*old_Q_val + lr*learned_Q_val
